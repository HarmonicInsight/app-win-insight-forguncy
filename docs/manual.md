...
1. **ファイル選択**: ドロップエリアに.fgcpファイルをドラッグ＆ドロップ（またはクリックして選択）
...
※ 初回起動時に認証ダイアログが表示されます。「Free版で続行」を選択すると制限付きで利用できます。

### 3.2 ライセンスアクティベーション

**初回起動時:**
1. アプリケーション起動時に認証ダイアログが表示されます
2. メールアドレスとライセンスキーを入力
3. 「認証」ボタンをクリック
4. 認証成功後、次回から自動認証

**ライセンスダイアログから:**
1. ヘッダーの「🔑 ライセンス」ボタンをクリック
2. メールアドレスを入力
3. ライセンスキーを入力
   - 形式: `FGIN-STD-2601-XXXX-XXXX-XXXX`
4. 「認証」ボタンをクリック

ライセンス情報はローカルに保存され、次回起動時に自動的に読み込まれます。

**重要:** ライセンスキーはメールアドレスと紐づいています。購入時に登録したメールアドレスと同じアドレスを入力してください。

### 3.3 ライセンスキー形式

```
FGIN-STD-2601-XXXX-YYYY-ZZZZ
│    │   │    │    │    └─ 署名パート2 (4文字)
│    │   │    │    └────── 署名パート1 (4文字)
│    │   │    └─────────── メールハッシュ (4文字)
│    │   └──────────────── 有効期限 (YYMM形式: 2026年1月)
...
| コード | プラン名 | 説明 |
|--------|---------|------|
| TRIAL | トライアル | 14日間の試用版 |
| STD | Standard | 年間ライセンス |
| PRO | Pro | 年間ライセンス（上位版） |

### 3.4 期限警告

有効期限の30日前から、起動時に更新のお知らせが表示されます。
